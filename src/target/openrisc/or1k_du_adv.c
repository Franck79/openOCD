...
/*
static int adbg_start_jtag_server(struct or1k_jtag *jtag_info)
{
	return ERROR_OK;
}
*/
static int or1k_adv_jtag_init(struct or1k_jtag *jtag_info)
{
	...
	if (or1k_du_adv.options & ENABLE_JSP_SERVER) {
		...
		retval = jsp_init(jtag_info, "-- JSP Telnet Server --");
		if (retval != ERROR_OK) {
				  ...
				  int *in_len, unsigned char *in_buffer)
{
	...
	if (or1k_du_adv.options & ENABLE_JSP_MULTI) {
		...
		startbit = 0;
		out_data[0] = 0x0 | (xmitsize << 4);  // First byte out has write count in upper nibble
		if (xmitsize > 0)
	...
	*in_len = (in_data[0] >> 4) & 0xF;  // bytes available is in the upper nibble
